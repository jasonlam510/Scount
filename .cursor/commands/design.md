...
handoffs:
  - label: Create UI Design
    ...
    send: true
  - label: Create Blueprint (tech-lead)
    agent: tech-lead
    prompt: Run /blueprint for the active feature scope using requirements.md (and uiux.md if present) to generate blueprint.md in the feature folder
    send: true
---
...
6. **After approval**:
   - If user responds with approval like "ok" or "please design":
     ...
     - Also hand off to `tech-lead` to run `/blueprint` to generate `FEATURE_DIR/blueprint.md`.
       - Note: `uiux.md` is optional input to `/blueprint`, so these can run independently.
   - After `uiux.md` and `blueprint.md` are ready, suggest: "Next: run `/plan` for tasks."
