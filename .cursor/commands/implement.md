...
- Detailed implementation instructions live in `FEATURE_DIR/tasks/*.md` (pseudo code per unit).
...
6. **Verify prerequisites (CRITICAL)**:
   ...
   - Ensure per-unit task files exist in `FEATURE_DIR/tasks/` and include "Changes (pseudo code)".
...
8. **Implement the unit**:
   ...
   - Read the unit file in `FEATURE_DIR/tasks/` and implement directly from its **Pseudo code**.
...
- **Pseudo-code-driven**: Follow the unit file pseudo code
